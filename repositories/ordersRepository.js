...
async function getOrdersByClientId(id){
    const result = await connection.query('SELECT orders.id as "orderId", orders.quantity, orders."createdAt", orders."totalPrice", orders."isDelivered" ,cakes.name as "cakeName", flavours.name as flavour FROM orders JOIN cakes ON orders."cakeId" = cakes.id JOIN flavours ON flavours.id = cakes."flavourId" WHERE orders."clientId" = $1',[id]);
    return result;
}
